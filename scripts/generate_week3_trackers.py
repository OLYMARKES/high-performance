...
SOURCE_TEMPLATE_PATH = ROOT / "week-3-tracker.html"
...
def get_week3_tracker_participants() -> list[dict[str, str]]:
...
def build_material_query(participant: dict[str, str]) -> str:
    tracker_url = f"{PUBLIC_BASE_URL}/w3_{participant['token']}.html?{TRACKER_VERSION_QUERY}"
    return urlencode(
        {
...
def add_personalization(template: str, participant: dict[str, str]) -> str:
    ...
    material_query = build_material_query(participant)
    ...
    html = html.replace("<title>Трекер недели 3 — High Performance</title>", f"<title>Трекер недели 3 — {name}</title>", 1)
    html = html.replace(
        '<div class="hero-tag">High Performance · Трекер · Апрель 2026</div>',
        f'<div class="hero-tag">High Performance · {name} · Неделя 3</div>',
        1,
    )
    html = html.replace(
        '<p class="hero-sub">Ценности, идентичность и новое пророчество. Каждый день — голос за того человека, которым ты выбираешь быть.</p>',
        f'<p class="hero-sub">Персональный трекер третьей недели для {for_name}. Здесь собраны тренировки, материалы недели, манифест и ежедневный компас для ценностей, идентичности и нового пророчества.</p>',
        1,
    )
    html = html.replace(
        "Оля Маркес · High Performance · Трекер недели 3 · Апрель 2026",
        f"{name} · High Performance · Трекер недели 3 · Апрель 2026",
        1,
    )
    html = html.replace(
        """.hero-sub {
...
/* ══════════════════════════════════
   ...
   ══════════════════════════════════ */""",
    ...
    html = html.replace(
        '    <div class="workout-section reveal" id="workoutSection"></div>',
        f"{build_week_switch_markup(token)}\n\n    <div class=\"workout-section reveal\" id=\"workoutSection\"></div>",
        1,
    )
    html = html.replace(
        'href="week-3.html" target="_blank" rel="noopener"',
        f'href="../week-3.html?{material_query}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
        'href="week-3-framework-values.html" target="_blank" rel="noopener"',
        f'href="../week-3-framework-values.html?{material_query}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
        'href="week-3-framework-identity.html" target="_blank" rel="noopener"',
        f'href="../week-3-framework-identity.html?{material_query}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
        'href="week-3-framework-prophecy.html" target="_blank" rel="noopener"',
        f'href="../week-3-framework-prophecy.html?{material_query}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
        "const FRAMEWORK_STORAGE_KEY = 'hp_week3_values_framework';",
        f"const FRAMEWORK_STORAGE_KEY = 'hp_week3_values_framework_{slug}';",
        1,
    )
    html = html.replace("'hp_week3_tracker'", f"'hp_week3_tracker_{slug}'")
    return html
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="robots" content="noindex, nofollow, noarchive">
  <title>HIGH PERFORMANCE — Private Week 3 Trackers</title>
  <style>
    body {
      min-height: 100vh;
      display: flex;
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="robots" content="noindex, nofollow, noarchive">
  <title>Week 3 Team Access</title>
  <style>
    body {{
      min-height: 100vh;
      margin: 0;
...
<body>
  <div class="page">
    <div class="hero">
      <h1>Week 3 Team Access</h1>
      <p>Командная страница со всеми персональными трекерами третьей недели. Эту ссылку не пересылаем участницам.</p>
    </div>
    <div class="panel">
      <div class="grid">
{cards_html}
...
def build_telegram_messages(participants: list[dict[str, str]]) -> str:
    ...
    for participant in participants:
        ...
        blocks.append(
            ...
            f"Открываю тебе трекер недели 3:\n{tracker_url}\n\n"
            f"Внутри уже есть тренировки, манифест, материалы по ценностям, идентичности и пророчеству. "
            f"Все ссылки недели живут внутри этой же персональной страницы.\n\n"
            f"Чат программы:\n{CHAT_URL}\n"
        )
    return "\n\n" + ("\n\n" + ("-" * 72) + "\n\n").join(blocks) + "\n"
...
def main() -> None:
    ...
    participants = get_week3_tracker_participants()

    for old_file in OUTPUT_DIR.glob("w3_*.html"):
        old_file.unlink()
    for stale_name in ("telegram_message.txt",):
        stale_file = OUTPUT_DIR / stale_name
        if stale_file.exists():
            stale_file.unlink()
    for old_team_page in OUTPUT_DIR.glob("week3-vault-*.html"):
        old_team_page.unlink()

    entries = []
    for participant in participants:
      filename = f"w3_{participant['token']}.html"
    ...
    print(f"Generated {len(entries)} week 3 trackers in {OUTPUT_DIR}")
