...
<script>
  ...
  function normalizeCompactTemplateItems(items) {{
    if (!Array.isArray(items) || !items.length) {{
      return [];
    }}
    ...
    return items.map((item, index) =>
      normalizeTrackerItem(
        {{
          ...
          title: item?.t,
          subtitle: item?.s,
          hasInput: Boolean(item?.h),
          inputPlaceholder: typeof item?.p === 'string' ? item.p : 'Запиши...',
          category: typeof item?.c === 'string' ? item.c : 'focus'
        }},
        `item_${{index}}`
      )
    );
  ...
  function expandStoredTrackerState(rawState) {{
    if (
      !rawState ||
    ...
    const templateItems = normalizeCompactTemplateItems(rawState.t);
    const days = DAY_NAMES.map((name, dayIndex) => {{
      const values = Array.isArray(rawState.d[dayIndex]) ? rawState.d[dayIndex] : [];
      const valuesById = new Map(
        values
          .filter((item) => item && typeof item === 'object')
          .map((item) => [canonicalItemId(item.i) || item.i, item])
      );

      return {{
        name,
        items: templateItems.map((templateItem, itemIndex) => {{
          const itemKey = canonicalItemId(templateItem.id) || templateItem.id;
          const saved = valuesById.get(itemKey);
          return {{
            ...cloneItemTemplate(templateItem, itemIndex),
            checked: Boolean(saved?.x),
            inputValue: typeof saved?.v === 'string' ? saved.v : ''
          }};
        }})
      }};
    }});
    ...
    return {{
      manifesto: typeof rawState.m === 'string' ? rawState.m : '',
      days
    }};
  }}

  function compactTrackerStateForStorage(rawState = state) {{
    const normalizedState = normalizeTrackerState(rawState);
    const templateItems = normalizeDayItems(normalizedState.days?.[0]?.items, true);

    return {{
      sv: 2,
      m: normalizedState.manifesto || '',
      t: templateItems.map((item) => {{
        ...
        if (item.subtitle) {{
          compactItem.s = item.subtitle;
        }}
        if (item.hasInput) {{
          compactItem.h = 1;
          if (item.inputPlaceholder && item.inputPlaceholder !== 'Запиши...') {{
            compactItem.p = item.inputPlaceholder;
          }}
        }}
        if (item.category && item.category !== 'focus') {{
          compactItem.c = item.category;
        }}
        return compactItem;
      }}),
      d: normalizedState.days.map((day) =>
        normalizeDayItems(day.items).reduce((entries, item) => {{
          if (!item.checked && !item.inputValue) {{
            return entries;
          }}

          const compactItem = {{
            i: canonicalItemId(item.id) || item.id
          }};
          if (item.checked) {{
            compactItem.x = 1;
          }}
          if (item.inputValue) {{
            compactItem.v = item.inputValue;
          }}
          entries.push(compactItem);
          return entries;
        }}, [])
      )
    }};
  ...
  function normalizeTrackerState(rawState) {{
    rawState = expandStoredTrackerState(rawState);
  ...
  function buildTrackerPayload(snapshotState = state, submittedAt = new Date().toISOString()) {{
    return {{
      ...
      weekKey: WEEK_KEY,
      trackerState: compactTrackerStateForStorage(snapshotState),
  ...
  function formatTrackerSaveError(error) {{
    const detail = String(error?.message || '');
    if (detail.includes('github_request_failed:503')) {{
      return 'Сервер сохранения временно перегружен. Данные остались в этом браузере, попробуй ещё раз через 10-20 секунд.';
    }}
    if (detail.includes('origin_not_allowed')) {{
      return 'Ссылка открыта в режиме, где сервер не принимает сохранение. Лучше открыть тот же трекер в обычном браузере или Telegram и попробовать снова.';
    }}
    if (detail.includes('missing_required_fields')) {{
      return 'В трекере пока нечего сохранять на сервер. Заполни хотя бы один пункт или манифест и попробуй снова.';
    }}
    return 'Не удалось сохранить серверную версию. Локальный черновик остался в этом браузере.';
  }}
  ...
  async function persistTrackerSnapshotInBackground(reason = 'change') {{
    ...
    try {{
      ...
      setSaveStatus(formatTrackerSaveError(error));
      return false;
    }} finally {{
  ...
  async function persistTrackerSnapshot(triggerButton, pendingStatus = 'Сохраняю трекер...') {{
    ...
    try {{
      ...
      const message = formatTrackerSaveError(error);
