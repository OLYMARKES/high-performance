...
<script>
  ...
  function expandStoredTrackerState(rawState) {{
    ...
    const templateItems = enrichTemplateWithWeek2Items(normalizeCompactTemplateItems(rawState.t));
  ...
  function buildWeek2StateFromWeek1(rawState) {{
    const source = expandStoredTrackerState(rawState && typeof rawState === 'object' ? rawState : {{}});
  ...
  (async () => {{
    ...
    if (!loadedWeek2 && !restoredLocal) {{
      const importedWeek1 = await importWeek1StateIfNeeded();
      if (importedWeek1 && hasUnsyncedServerChanges()) {{
