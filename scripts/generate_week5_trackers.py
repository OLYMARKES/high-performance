...
SOURCE_TEMPLATE_PATH = ROOT / "week-5-tracker.html"
OUTPUT_DIR = ROOT / "week_5_trackers_april_2026"
PUBLIC_BASE_URL = "https://olymarkes.github.io/high-performance/week_5_trackers_april_2026"
TRACKER_VERSION_QUERY = "v=week5-rollout-v1"
TEAM_PAGE_TOKEN = "week5-vault-m8q2t6k4p1x7"
...
def build_week_switch_markup(token: str) -> str:
    return f"""    <nav class="week-switch reveal" aria-label="Переключение недели">
      ...
      <a class="week-switch-btn" href="../week_4_trackers_april_2026/w4_{token}.html?{TRACKER_VERSION_QUERY}">Неделя 4</a>
      <a class="week-switch-btn is-active" href="../week_5_trackers_april_2026/w5_{token}.html?{TRACKER_VERSION_QUERY}">Неделя 5</a>
    </nav>"""
...
def add_personalization(template: str, participant: dict[str, str]) -> str:
    ...
    html = html.replace("<title>Трекер недели 5 — High Performance</title>", f"<title>Трекер недели 5 — {name}</title>", 1)
    html = html.replace(
        '<div class="hero-tag">High Performance · Финальная неделя · 27 апреля — 3 мая</div>',
        f'<div class="hero-tag">High Performance · {name} · Неделя 5</div>',
        1,
    )
    html = html.replace(
        '<p class="hero-sub">Итог месяца и дизайн следующего. Фокус — на том, что уже получилось, и на трёх-четырёх опорах, с которыми я захожу в май.</p>',
        f'<p class="hero-sub">Персональный трекер пятой недели для {for_name}. Здесь можно постепенно собирать материалы финальной недели, держать дневной ритм и фиксировать вход в следующий месяц.</p>',
        1,
    )
    html = html.replace(
        """.hero-sub {
...
/* ══════════════════════════════════
   DAY NAV""",
    ...
    html = html.replace(
        'href="week-1-tracker.html" target="_blank"',
        f'href="../week_1_trackers_april_2026/w1_{token}.html?{TRACKER_VERSION_QUERY}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
    ...
    html = html.replace(
        'href="week-4-tracker.html" target="_blank"',
        f'href="../week_4_trackers_april_2026/w4_{token}.html?{TRACKER_VERSION_QUERY}" target="_blank" rel="noopener"',
        1,
    )
    html = html.replace(
        "Оля Маркес · High Performance · Трекер недели 5 · 27 апреля — 3 мая 2026",
        f"{name} · High Performance · Трекер недели 5 · 27 апреля — 3 мая 2026",
        1,
    )
    html = html.replace("'hp_week5_tracker'", f"'hp_week5_tracker_{slug}'", 1)
    return html
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="robots" content="noindex, nofollow, noarchive">
  <title>HIGH PERFORMANCE — Private Week 5 Trackers</title>
  <style>
    body {
      min-height: 100vh;
      display: flex;
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="robots" content="noindex, nofollow, noarchive">
  <title>Week 5 Team Access</title>
  <style>
    body {{
      min-height: 100vh;
      margin: 0;
...
<body>
  <div class="page">
    <div class="hero">
      <h1>Week 5 Team Access</h1>
      <p>Командная страница со всеми персональными трекерами пятой недели. Эту ссылку не пересылаем участницам.</p>
    </div>
    <div class="panel">
      <div class="grid">
{cards_html}
...
def build_telegram_messages(participants: list[dict[str, str]]) -> str:
    ...
    for participant in participants:
        tracker_url = f"{PUBLIC_BASE_URL}/w5_{participant['token']}.html?{TRACKER_VERSION_QUERY}"
        blocks.append(
            f"=== {participant['display_name']} · {participant['telegram_handle']} ===\n"
            f"Привет, {participant['public_name']}!\n\n"
            f"Открываю тебе трекер недели 5:\n{tracker_url}\n\n"
            f"Внутри уже есть материалы финальной недели, недельный фокус и ежедневный трекер с автосохранением.\n\n"
            f"Чат программы:\n{CHAT_URL}\n"
        )
    return "\n\n" + ("\n\n" + ("-" * 72) + "\n\n").join(blocks) + "\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate week 5 trackers")
    parser.add_argument("--slug", action="append", dest="slugs", help="Generate only for selected participant slug(s)")
...
def main() -> None:
    ...
    if args.slugs:
        ...
        participants = [participant for participant in participants if participant["slug"] in wanted]
        if not participants:
            raise SystemExit(f"No participants found for slugs: {', '.join(sorted(wanted))}")
    else:
        for old_file in OUTPUT_DIR.glob("w5_*.html"):
            old_file.unlink()
        for stale_name in ("telegram_message.txt",):
            stale_file = OUTPUT_DIR / stale_name
            if stale_file.exists():
                stale_file.unlink()
        for old_team_page in OUTPUT_DIR.glob("week5-vault-*.html"):
            old_team_page.unlink()

    entries = []
    for participant in participants:
        filename = f"w5_{participant['token']}.html"
    ...
    print(f"Generated {len(entries)} week 5 tracker(s) in {OUTPUT_DIR}")
